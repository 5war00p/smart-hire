...
const TabContent: FC<{ tab: number }> = ({ tab }) => {
  switch (tab) {
    case 1:
      return (
        <div className="flex gap-4 max-w-xs md:max-w-lg">
          {Array.from(Array(1)).map((each) => (
            <div
              key={each}
    ...
    case 2:
      return (
        <div className="flex gap-4 max-w-xs md:max-w-lg">
          {Array.from(Array(1)).map((each, index) => (
            <div
              key={index}
    ...
    case 3:
      return (
        <div className="flex gap-2 max-w-xs md:max-w-md flex-wrap">
          {Array.from(Array(50)).map((each, index) => (
            <span
              key={index}
...
const MessageCard: FC<{ details: Record<string, any> }> = ({ details }) => {
  return (
    <div className="rounded-lg drop-shadow-md border-gray-200 bg-white px-4 py-5 sm:px-6 min-w-[512px] space-y-4">
      <div className="flex flex-col sm:flex-row min-w-0 gap-4">
        ...
        <div className="min-w-0 flex-auto">
          ...
          <p className="mt-1 truncate text-xs text-gray-500">+1-752-676-3840</p>
          <p className="mt-1 truncate text-xs text-gray-500">Charlotte, USA</p>
        </div>
      </div>
      <Tabs />
    </div>
